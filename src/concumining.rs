...
pub fn start(total_miners: usize, rounds: usize) {
    ...
    leader_loop(total_miners, rounds, leader_rx, &mut senders, condvar_listen, condvar_transfer);
...
fn leader_loop(
    ...
    for _ in 0..rounds {
        ...
        give_orders(total_miners + 1, &senders, Commands::EXPLORE);

        //Esperamos 2 segundos para dar la orden de regreso
        thread::sleep(time::Duration::from_millis(2000));

        //Ordenamos a los mineros a volver
        give_orders(total_miners + 1, &senders, Commands::RETURN);
    ...
    give_orders(total_miners + 1, &senders, Commands::STOP);
}

fn give_orders(leader_id: usize, senders: &HashMap<usize, Sender<Message>>, command: Commands) {
